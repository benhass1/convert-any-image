...
const ViewExif = lazy(() => import("./pages/ViewExif"));
...
function LoadingPage() { return <main className="grid min-h-screen place-items-center bg-[#f7f4ee] px-5 text-center" aria-label="Loading page"><div><span className="mx-auto grid h-12 w-12 place-items-center border border-[#132432] bg-[#b7f840] text-xs font-bold text-[#132432]">CAI</span><p className="label mt-5">PREPARING YOUR PAGE</p><p className="mt-2 text-sm text-[#52616a]">Loading the guide workspace.</p></div></main>; }
 function Router() { return <Suspense fallback={<LoadingPage/>}><ScrollManager/><Switch><Route path="/" component={Home} /><Route path="/fr/" component={Home} /><Route path="/it/" component={Home} /><Route path="/ja/" component={Home} /><Route path="/pl/" component={Home} /><Route path="/nl/" component={Home} /><Route path="/de/" component={Home} /><Route path="/es/" component={Home} /><Route path="/ko/" component={Home} /><Route path="/ru/" component={Home} /><Route path="/pt/" component={Home} /><Route path="/heic-to-jpg">{() => <FormatToolPage variant="heic"/>}</Route><Route path="/webp-to-png">{() => <FormatToolPage variant="webp"/>}</Route><Route path="/compress" component={Compress}/><Route path="/remove-exif" component={RemoveExif}/><Route path="/view-exif" component={ViewExif}/><Route path="/blog" component={BlogIndex}/><Route path="/blog/:slug" component={BlogArticle}/><Route path="/about" component={About}/><Route path="/privacy">{() => <Legal kind="privacy"/>}</Route><Route path="/terms">{() => <Legal kind="terms"/>}</Route><Route path="/cookie-policy">{() => <Legal kind="cookies"/>}</Route><Route path="/404" component={NotFound} /><Route component={NotFound} /></Switch></Suspense>; }
