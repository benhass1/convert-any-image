...
export function BlogIndex() {
  ...
  return <SiteShell><Seo exactTitle title="Image Conversion & Compression Guides" description="Practical guides for converting image formats, reducing file size and keeping supported image processing private on your device." keywords="image conversion guides, image compression guides, PSD to JPG, HEIC to JPG, RAW to JPG, reduce image size"/><main>
    <section className="bg-[#132432] px-5 py-16 text-[#f4f0e8] lg:px-10 lg:py-24"><div className="mx-auto grid max-w-[1220px] gap-10 lg:grid-cols-[.8fr_1.2fr]"><aside className="border-l border-[#b7f840] pl-4"><p className="label text-[#b7f840]">PRACTICAL GUIDES</p><p className="mt-5 text-[.63rem] font-bold tracking-[.16em] text-[#d8d2c5]/55">INDEX / {articles.length.toString().padStart(2, "0")} NOTES</p><div className="mt-6 grid h-10 w-10 place-items-center border border-[#b7f840] text-[.64rem] font-bold text-[#b7f840]">HOW</div></aside><div><h1 className="font-display text-5xl font-bold leading-[.94] tracking-[-.075em] lg:text-7xl">Fix the format.<br/>Keep control of the file.</h1><p className="mt-6 max-w-xl text-lg leading-7 text-[#d8d2c5]/74">Step-by-step notes for format problems, browser-safe conversion choices and image-size decisions that make sense for the destination.</p></div></div></section>
    <section className="mx-auto max-w-[1220px] px-5 py-12 lg:px-10 lg:py-16">{grouped.map((group) => <section key={group.category} className="mb-16 last:mb-0"><div className="mb-6 flex items-end justify-between gap-5 border-b border-[#132432]/12 pb-4"><div><p className="label">TOPIC CLUSTER</p><h2 className="font-display mt-2 text-3xl font-bold tracking-[-.06em]">{group.category}</h2></div><span className="text-[.65rem] font-bold tracking-[.14em] text-[#65727b]">{group.entries.length.toString().padStart(2, "0")} GUIDES</span></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{group.entries.map((article, index) => <GuideCard key={article.slug} article={article} index={index}/>)}</div></section>)}</section>
  </main></SiteShell>;
}
