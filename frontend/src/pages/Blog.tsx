...
const articleSchema = (article: Article) => ({
  ...
  author: { "@type": "Organization", "@id": "https://convertanyimage.com/#organization", name: "Convert Any Image Editorial Team", url: "https://convertanyimage.com/about" },
  publisher: { "@type": "Organization", "@id": "https://convertanyimage.com/#organization", name: "Convert Any Image", logo: { "@type": "ImageObject", url: "https://convertanyimage.com/favicon.svg" } },
  datePublished: "2026-08-27T00:00:00+02:00",
...
const guideIndexSchema = {
  "@context": "https://schema.org",
  "@graph": [
    { "@type": "CollectionPage", "@id": "https://convertanyimage.com/blog#webpage", name: "Image Conversion & Compression Guides", url: "https://convertanyimage.com/blog", description: "Practical guides for image formats, compression and image-privacy decisions.", author: { "@type": "Organization", "@id": "https://convertanyimage.com/#organization", name: "Convert Any Image Editorial Team", url: "https://convertanyimage.com/about" }, publisher: { "@type": "Organization", "@id": "https://convertanyimage.com/#organization", name: "Convert Any Image" }, datePublished: "2026-08-27T00:00:00+02:00", dateModified: "2026-08-27T00:00:00+02:00", citation: ["https://www.cipa.jp/e/std/std-sec.html", "https://www.loc.gov/preservation/digital/formats/fdd/fdd000618.shtml"] },
  ],
};
...
export function BlogArticle() {
  ...
  const isExif = article.tool === "exif";
  return <SiteShell><Seo exactTitle title={article.metaTitle} description={article.description} keywords={article.keyword} jsonLd={articleSchema(article)}/><main><article className="mx-auto max-w-[1040px] px-5 py-14 lg:py-20">
    ...
    <div className="mt-14 grid gap-8 lg:grid-cols-[185px_minmax(0,1fr)]"><aside className="h-fit border-l-2 border-[#b7f840] bg-[#132432] p-5 text-[#f4f0e8] lg:sticky lg:top-28"><span className="grid h-9 w-9 place-items-center border border-[#b7f840] text-[.6rem] font-bold text-[#b7f840]">GUIDE</span><p className="label mt-6 text-[#b7f840]">{article.category}</p><p className="mt-3 flex items-center gap-2 text-sm text-[#d8d2c5]/72"><Clock3 className="h-4 w-4"/>{article.reading}</p><p className="mt-7 border-t border-[#f4f0e8]/15 pt-4 text-[.6rem] font-bold tracking-[.14em] text-[#d8d2c5]/48">LOCAL FIELD NOTE</p></aside><div className="relative"><span className="absolute -left-3 -top-3 h-6 w-6 border-l border-t border-[#b7f840]"/><p className="label text-[#5c7820]">{article.keyword}</p><h1 className="font-display mt-3 text-5xl font-bold leading-[.94] tracking-[-.075em] lg:text-7xl">{article.title}</h1><p className="mt-4 text-sm font-semibold text-[#52616a]">By <Link href="/about" className="underline decoration-[#b7f840] underline-offset-4">Convert Any Image Editorial Team</Link> · <time dateTime="2026-08-27T00:00:00+02:00">Updated August 27, 2026</time></p><p className="mt-7 max-w-3xl font-serif text-2xl leading-9 text-[#41525d]">{article.lead} Use the <a href={isExif ? "/remove-exif" : article.tool === "compress" ? "/compress" : "/webp-to-png"} className="font-semibold text-[#132432] underline decoration-[#b7f840] underline-offset-4" title={isExif ? "remove EXIF data from photos free" : article.tool === "compress" ? "compress PNG for Discord without losing quality" : "convert WebP to PNG for Shopify transparent backgrounds"}>
  {isExif ? "remove EXIF data from photos free" : article.tool === "compress" ? "compress PNG for Discord without losing quality" : "convert WebP to PNG for Shopify transparent backgrounds"}
</a> when a compatible local processing path is available.</p><GuideFigure visual={article.visual} fallbackAlt={`${article.title} step-by-step visual guide`}/>{article.sections.map((section, index) => <section key={section.heading} className="mt-12"><h2 className="font-display text-3xl font-bold leading-[.98] tracking-[-.06em]">{section.heading}</h2><div className="mt-5 space-y-5 font-serif text-lg leading-8 text-[#314753]">{section.paragraphs.map((paragraph) => <p key={paragraph} dangerouslySetInnerHTML={{ __html: paragraph }}/>)}</div>{section.bullets && <ul className="mt-6 space-y-3 border-l-2 border-[#b7f840] pl-5 text-sm leading-6 text-[#41525d]">{section.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul>}{index === 0 && <ToolCta compact tool={article.tool}/>}</section>)}{article.category === "PRIVACY & SECURITY" && <section className="mt-12 border-y border-[#132432]/12 py-8"><p className="label text-[#5c7820]">SOURCES &amp; STANDARDS</p><h2 className="font-display mt-3 text-2xl font-bold tracking-[-.05em]">External references</h2><ul className="mt-4 space-y-3 text-sm leading-6 text-[#41525d]"><li><a className="font-semibold underline decoration-[#b7f840] underline-offset-4" href="https://www.loc.gov/preservation/digital/formats/fdd/fdd000618.shtml">Library of Congress: Exchangeable Image File Format (Exif) Family</a></li><li><a className="font-semibold underline decoration-[#b7f840] underline-offset-4" href="https://www.cipa.jp/e/std/std-sec.html">Camera &amp; Imaging Products Association: Exif standards</a></li></ul></section>}<p className="mt-10 text-lg leading-8 text-[#314753]">When the next step is about image size, use the <a href="/compress#compress-upload" className="font-semibold text-[#132432] underline decoration-[#b7f840] underline-offset-4">image compressor with adjustable quality and dimensions</a> and review the before-and-after size before downloading.</p><ToolCta tool={article.tool}/>{related.length > 0 && <section className="mt-14 border-t border-[#132432]/12 pt-9"><p className="label">RELATED GUIDES</p><div className="mt-5 grid gap-3 sm:grid-cols-2">{related.map((entry) => <Link key={entry.slug} href={`/blog/${entry.slug}`} className="group border border-[#132432]/12 bg-[#f7f4ee] p-5 transition-colors hover:bg-[#e8f4cc]"><p className="label text-[#5c7820]">{entry.category}</p><h3 className="font-display mt-3 text-xl font-bold tracking-[-.04em]">{entry.title}</h3><span className="mt-5 flex items-center gap-2 text-sm font-bold">Read guide <ArrowUpRight className="h-4 w-4 transition-transform group-hover:translate-x-1"/></span></Link>)}</div></section>}</div></div>
